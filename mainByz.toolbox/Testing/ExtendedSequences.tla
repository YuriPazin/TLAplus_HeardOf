...
BagIt(f,e) == [x \in AllOf(f,"x") |-> Count(Keep(Only(f,e),x))]

MostOf(f,e) ==  LET eB == BagIt(f,e)
                IN  {x \in DOMAIN eB:
                    \A y \in DOMAIN eB:
                    eB[x] >= eB[y]}

MoreThan(f,e,E) == \E x \in DOMAIN BagIt(f,e): BagIt(f,e)[x] > E 
...
\* Last modified Fri Nov 15 20:30:43 BRT 2024 by yuri
